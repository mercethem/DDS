...
if ! bash "$SCRIPT_DIR/find_tools.sh"; then
    echo -e "${RED}[ERROR] Tool detection failed!${NC}"
    exit 1
fi

# Load detected environment
ENV_FILE="$SCRIPT_DIR/export_environment_vars.sh"
if [[ -f "$ENV_FILE" ]]; then
    source "$ENV_FILE"
    echo -e "${GREEN}[OK] Environment loaded successfully${NC}"
else
    echo -e "${RED}[ERROR] Environment file not found: $ENV_FILE${NC}"
    echo -e "${YELLOW}[INFO] Attempting to create environment file manually...${NC}"
    # Fallback: create basic environment file
    cat > "$ENV_FILE" << EOF
#!/bin/bash
...
EOF
    source "$ENV_FILE"
    echo -e "${GREEN}[OK] Environment file created and loaded${NC}"
fi
...
FULL_ENV_FILE="$PROJECT_ROOT/init/sh/export_dds_environment.sh"
mkdir -p "$PROJECT_ROOT/init/sh"
...
ALIASES_FILE="$PROJECT_ROOT/init/sh/setup_dds_aliases.sh"
mkdir -p "$PROJECT_ROOT/init/sh"
