...
Start: scripts/patch_qos_settings.bat

Features:
- Module and variation (Publisher/Subscriber) selection (multi-select, scrollable)
- Simple mode: History only
- Advanced mode: All supported QoS fields for Reader/Writer
- Apply: Applies idempotent QoS patch to selected files (uses scripts/py/qos_settings_patcher.py)
"""
...
# Set sys.path to import patcher module
...
try:
    import qos_settings_patcher as qp
        ...
        messagebox.showerror("Import Error", f"qos_settings_patcher.py could not be imported: {e}")
    raise
...
class QosPatcherGUI(tk.Tk):
    def __init__(self) -> None:
        ...
        # UI (grid-based, full-screen responsive)
        self._build_ui()

    def _build_ui(self) -> None:
        # Main grid configuration (0..4 rows)
        for r in range(5):
            self.rowconfigure(r, weight=(1 if r in (0, 3) else 0))
        self.columnconfigure(0, weight=1)

        # Top section: Module list + Variation selections
        top = ttk.Frame(self)
        top.grid(row=0, column=0, sticky="nsew", padx=10, pady=10)

        ttk.Label(top, text="Modules").grid(row=0, column=0, sticky="w")
        ...
        role_frame = ttk.LabelFrame(top, text="Variation")
        ...
        ttk.Radiobutton(mode_frame, text="Simple (History only)", value="simple", variable=self.mode).pack(anchor="w", padx=10, pady=5)
        ttk.Radiobutton(mode_frame, text="Advanced (Reader/Writer all QoS)", value="advanced", variable=self.mode).pack(anchor="w", padx=10)
        ...
        self.hist_kind = tk.StringVar(value="Select")
        self.hist_kind_cb = ttk.Combobox(simple_frame, values=["Select", "KEEP_LAST", "KEEP_ALL"], textvariable=self.hist_kind, state="readonly", width=20)
        ...
        # Advanced QoS area
        # Advanced QoS area (scrollable)
        adv_container = ttk.LabelFrame(self, text="Advanced QoS Selections")
        ...
        ttk.Button(btns, text="Cancel", command=self.on_cancel).pack(side=tk.RIGHT, padx=5)
        ttk.Button(btns, text="Save", command=self.on_save).pack(side=tk.RIGHT)

        # Disable advanced area initially
    ...
    def _build_advanced_section(self, parent: ttk.LabelFrame) -> None:
        ...
        self.w_rel = tk.StringVar(value="Select")
        self.w_dur = tk.StringVar(value="Select")
        self.w_liv = tk.StringVar(value="Select")
        self.w_dst = tk.StringVar(value="Select")
        self.w_own = tk.StringVar(value="Select")
        ...
        self.w_rel_cb = self._row_writer(wf, "Reliability", self.w_rel, ["Select"] + rel_opts, 0)
        self.w_dur_cb = self._row_writer(wf, "Durability", self.w_dur, ["Select"] + dur_opts, 1)
        self.w_liv_cb = self._row_writer(wf, "Liveliness", self.w_liv, ["Select"] + liv_opts, 2)
        self.w_dst_cb = self._row_writer(wf, "DestinationOrder", self.w_dst, ["Select"] + dst_opts, 3)
        self.w_own_cb = self._row_writer(wf, "Ownership", self.w_own, ["Select"] + own_opts, 4)
        ...
        ttk.Checkbutton(wf, text="Set ResourceLimits", variable=self.w_rl_enable).grid(row=6, column=0, sticky="e", padx=6, pady=4)
        ...
        self.r_rel = tk.StringVar(value="Select")
        self.r_dur = tk.StringVar(value="Select")
        self.r_liv = tk.StringVar(value="Select")
        self.r_dst = tk.StringVar(value="Select")
        self.r_own = tk.StringVar(value="Select")
        self.r_rl_max = tk.StringVar(value="")
        self.r_rl_alloc = tk.StringVar(value="")
        self.r_rl_enable = tk.BooleanVar(value=False)

        self.r_rel_cb = self._row_reader(rf, "Reliability", self.r_rel, ["Select"] + rel_opts, 0)
        self.r_dur_cb = self._row_reader(rf, "Durability", self.r_dur, ["Select"] + dur_opts, 1)
        self.r_liv_cb = self._row_reader(rf, "Liveliness", self.r_liv, ["Select"] + liv_opts, 2)
        self.r_dst_cb = self._row_reader(rf, "DestinationOrder", self.r_dst, ["Select"] + dst_opts, 3)
        self.r_own_cb = self._row_reader(rf, "Ownership", self.r_own, ["Select"] + own_opts, 4)
        ttk.Checkbutton(rf, text="Set ResourceLimits", variable=self.r_rl_enable).grid(row=5, column=0, sticky="e", padx=6, pady=4)
    ...
    def on_save(self) -> None:
        ...
        if not modules:
            messagebox.showwarning("Selection", "Select at least one module")
            return
        roles = self._selected_roles()
        if roles == "none":
            messagebox.showwarning("Selection", "Select at least one variation (Publisher/Subscriber)")
            return
        ...
        if not chosen_files:
            messagebox.showinfo("Info", "No files found matching criteria")
            return
        ...
        if self.mode.get() == "simple":
            ...
            if hkind not in ("KEEP_LAST", "KEEP_ALL"):
                messagebox.showerror("Error", "History.kind invalid")
                return
            if hkind == "KEEP_LAST":
                try:
                    ...
                    messagebox.showerror("Error", "History.depth must be a positive integer")
                    return
        ...
        if not ok:
            messagebox.showerror("Validation", f"QoS validation error: {reason}")
            return
    ...
    # Auto-fill: scan existing QoS when module/role selections change
    def _autofill_from_code(self) -> None:
        modules = self._selected_modules()
        roles = self._selected_roles()
        if not modules or roles == "none":
            return
        # Base on first matching file (for UI simplicity)
        chosen_files = []
        for m in modules:
            rmap = self.grouped.get(m, {})
            if roles in ("publisher", "both") and "publisher" in rmap:
        ...
        if writer_file:
            ...
            # History (single UI field, also show in configuration)
    ...
    def _bind_autofill(self) -> None:
        # Auto-fill when module list and role checks change
        ...
        def _sync_numeric_state(*args):
            ...
            wos_state = tk.NORMAL if self.w_own.get() != "Select" else tk.DISABLED
            try:
    ...
    def _attach_tooltips(self) -> None:
        ...
        tips = {
            "Reliability": "Data delivery guarantee. RELIABLE: packet loss detected and compensated. BEST_EFFORT: low latency, loss possible.",
            "Durability": "Data persistence for new subscribers. VOLATILE: history not provided. TRANSIENT_LOCAL: cached history on writer side can be provided.",
            "Liveliness": "How publisher/subscriber liveliness is verified. AUTOMATIC or MANUAL_BY_TOPIC/PARTICIPANT.",
            "DestinationOrder": "Delivery order. By source timestamp (BY_SOURCE) or reception time (BY_RECEPTION).",
            "Ownership": "Ownership on same instance: SHARED (multiple writers) or EXCLUSIVE (single writer).",
        }
