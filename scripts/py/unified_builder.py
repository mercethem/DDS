...
class UnifiedBuilder:
    ...
    def find_executables(self) -> List[Path]:
        """Build edilen executable'ları bulur."""
        executables = []
        
        # Build dizininde executable'ları ara
        if self.build_dir.exists():
            for root, dirs, files in os.walk(self.build_dir):
                for file in files:
                    file_path = Path(root) / file
                    
                    # Windows için .exe, Linux için executable bit
    ...
    def full_build(self, build_type: str = "Release", clean: bool = False) -> bool:
        ...
        for exe in executables[:10]:  # İlk 10'unu göster
            print(f"  📄 {exe.name}")
