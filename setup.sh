...
# Step 0: Run dynamic_ALL.sh first (initial setup)
echo "========================================"
echo "STEP 0: Dynamic DDS Workflow (Initial Setup)"
echo "========================================"

if [ ! -f "$PROJECT_ROOT/scripts/sh/dynamic_ALL.sh" ]; then
    print_error "dynamic_ALL.sh not found: $PROJECT_ROOT/scripts/sh/dynamic_ALL.sh"
    exit 1
fi

# Make script executable
chmod +x "$PROJECT_ROOT/scripts/sh/dynamic_ALL.sh"

print_info "Starting Dynamic DDS workflow..."
# Note: dynamic_ALL.sh contains interactive prompts for testing
# If you want non-interactive mode, modify dynamic_ALL.sh or use expect/auto-answer
# dynamic_ALL.sh must be run from scripts/sh directory
(cd "$PROJECT_ROOT/scripts/sh" && bash dynamic_ALL.sh)
if [ $? -eq 0 ]; then
...
if [ $RECREATE_CERTS -eq 1 ]; then
    ...
    if [ ! -f "scripts/py/certificate.py" ]; then
        print_error "certificate.py not found!"
        exit 1
    fi
    
    # Run certificate generation
    if python3 scripts/py/certificate.py; then
...
if [ -f "scripts/py/clean_duplicates.py" ]; then
    if python3 scripts/py/clean_duplicates.py > /dev/null 2>&1; then
        print_success "Duplicate code blocks cleaned"
    else
        print_warning "Cleanup script executed (result pending)"
    fi
else
    print_warning "clean_duplicates.py not found (skipping...)"
fi
...
if [ -f "scripts/py/fix_cmake_portability.py" ]; then
    if python3 scripts/py/fix_cmake_portability.py > /dev/null 2>&1; then
        print_success "CMake files made portable"
    else
        print_warning "CMake fix executed (result pending)"
    fi
else
    print_warning "fix_cmake_portability.py not found (skipping...)"
fi
...
if [ -f "scripts/py/security.py" ]; then
    if python3 scripts/py/security.py > /dev/null 2>&1; then
        print_success "Security settings checked"
    else
        print_warning "Warning occurred while applying security settings (continuing...)"
    fi
else
    print_warning "security.py not found (skipping...)"
fi
...
if [ $NEED_BUILD -eq 1 ]; then
    ...
    if [ ! -f "scripts/sh/IDL_BUILDER.sh" ]; then
        print_error "IDL_BUILDER.sh not found!"
        exit 1
    fi
    
    # Make script executable
    chmod +x scripts/sh/IDL_BUILDER.sh
    
    # Run builder
    if bash scripts/sh/IDL_BUILDER.sh; then
        print_success "Build completed"
    else
        print_error "Build failed!"
        echo
        print_warning "To build manually:"
        echo "  scripts/sh/IDL_BUILDER.sh"
        exit 1
    fi
else
    echo "========================================"
...
if [ ! -f "$CA_CERT" ]; then
    ...
    if [ -f "$PARTICIPANT_CERT" ]; then
        ...
        print_info "certificate.py should be run in STEP 1 to create certificate"
    fi
fi
