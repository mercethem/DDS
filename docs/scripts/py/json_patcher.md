...
`json_patcher.py` modifies DDS Publisher applications to read data from JSON scenario files instead of using hardcoded values. It replaces the AUTOGENERATED IDL PATCH blocks with JSON file reading logic.
...
1. First: `idl_patcher.py` - Creates AUTOGENERATED blocks with default data
2. Then: `json_patcher.py` - Replaces blocks with JSON reading logic
...
- `setup.sh` - During patching phase
- `scripts/sh/json_patcher.sh` - Standalone execution
