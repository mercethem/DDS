# idl_default_data_patcher.py

## Overview

`idl_default_data_patcher.py` is an advanced IDL parser and code generator that parses IDL files and patches PublisherApp files with default data assignments. It uses a two-stage parsing approach and smart data generation based on field names and types.
...
```bash
cd scripts/py
python3 idl_default_data_patcher.py
```
