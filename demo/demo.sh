...
# Linux equivalent of Windows demo.bat file
...
if [ "$NODEJS_AVAILABLE" = true ] && [ ! -d "node_modules" ]; then
...
start_process() {
    ...
    eval "$command" &
    local pid=$!
    echo "$title PID: $pid" >> "$SCRIPT_DIR/processes.pid"
...
if [ "$NODEJS_AVAILABLE" = true ]; then
    ...
    start_process "Backend Server" "node server.js" "$SCRIPT_DIR"
    sleep 3
else
...
echo "- CoreData Publisher: ../IDL/CoreData_idl_generated/CoreDatamain publisher"
echo "- Intelligence Publisher: ../IDL/Intelligence_idl_generated/Intelligencemain publisher"  
echo "- Messaging Publisher: ../IDL/Messaging_idl_generated/Messagingmain publisher"
echo "- Monitor: ../monitoring/start_monitor.sh"
echo
